...
fn main() {
    ...
    let mut number = Rc::new(RefCell::new(get_tree(&args[0])));
    for string in args[1..].iter() {
        let added = Rc::new(RefCell::new(Number::Pair(
            number.clone(),
            Rc::new(RefCell::new(get_tree(string))),
        )));
        reduce_number(added.clone());
        number = added;
    }
