...
let line_errors l1 l2 =
  List.fold_left2 (fun acc x1 x2 -> acc + (if x1 = x2 then 0 else 1)) 0 l1 l2

let errors l1 l2 =
  List.fold_left2 (fun acc x1 x2 -> acc + (line_errors x1 x2)) 0 l1 l2
...
let get_mirror_index map =
  List.fold_left (fun (i, acc) (line1, line2) ->
    (i+1, if line_errors line1 line2 <= 1 then i::acc else acc)) (1, [])
...
let filter_mirror_index indexes lava =
  ...
  List.filter (fun index ->
    ...
    (errors trunc_lava (List.rev trunc_lava)) = 2) indexes
