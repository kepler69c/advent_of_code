...
let diff bricks1 bricks2 =
  let bricks1 = List.sort compare bricks1 in
  let bricks2 = List.sort compare bricks2 in
  List.fold_left (fun acc (b1, b2) ->
    acc + if b1 = b2 then 0 else 1) 0 (List.combine bricks1 bricks2)
...
let () =
  ...
  let ret = List.fold_left (fun acc b ->
    ...
    acc + (diff rem_bricks (gravity rem_bricks))) 0 ground_bricks in
  Printf.printf "%d\n" ret
